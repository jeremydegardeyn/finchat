...
│   ├── process/                   # Process APIs — cross-domain capabilities (ADR-0030)
│   └── experience/mobile/         # Mobile experience API — one screen, one round trip
...
| — | Model & Agent Inventory (FinChat AI control framework) | [`docs/19`](docs/19-model-inventory.md) |
| — | Agent Registry & Identity | [`docs/20`](docs/20-agent-registry.md) |
...
| — | **API-led layering** (system · process · experience) | [`docs/28`](docs/28-api-led-layering.md) |
...
- [x] **Increment 19 — Durable agent harness (Reconciliation Steward):** a new long-running data product (`products/steward/`) on a **durable-execution engine** — a **planner → generator → evaluator** loop that **sleeps at zero cost**, **survives crashes** (resumes from the last checkpoint — no lost work, no double side-effects), and **wakes on the approver / events with no polling**. Unlike the Loan workflow (a *fixed* Cloud Workflows route, Inc 4), the **agent owns** the loop and the plan is generated at runtime; the inline evaluator reuses the **live-eval LLM-judge** ([ADR-0015](docs/adr/0015-live-evaluation.md)) to gate every step and escalate to the **verified GIS approver** ([ADR-0016](docs/adr/0016-identity-resolved-personas.md)), writing an append-only audit. Dual-tier ([ADR-0021](docs/adr/0021-durable-agent-harness.md)): deploy tier = **DBOS on Cloud Run scale-to-zero + Cloud SQL Postgres** (`infra/modules/agent_harness`, toggle `enable_agent_harness` default **off**); enterprise 1:1 = **Temporal** (documented). ✅ offline reasoning tests + durable sleep / escalate-wake / **crash-recovery** verified on Postgres. ([docs/18](docs/18-durable-agent-harness.md))
- [x] **Increment 18 — End-user credential propagation:** Ask-the-Data queries now run **as the signed-in user** (SPA OAuth token → BFF → Conversational Analytics), so BigQuery evaluates each user’s own IAM + policy tags: the approver sees **real values** (fine-grained reader), the analyst sees **masked NULLs** (BigQuery **data-masking policy** on PII_FINANCIAL — the docs/09 roadmap item, now real), and the platform admin gets a **CLS denial mapped to a friendly “request access” message linking the Dataplex data-product access-request flow**. Ask-the-Data extended to approver + admin personas (persona gates the endpoint; the user’s credentials gate the data). ([ADR-0019](docs/adr/0019-end-user-credential-propagation.md))
