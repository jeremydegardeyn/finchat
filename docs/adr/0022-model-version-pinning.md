...
**Snapshot ids are not set in code.** They must be confirmed against the Model Garden
listing for the region in use and they change as versions retire; inventing one would
produce a config that fails at runtime and a document that reads as verified when it is
not. Until a pin is configured, the call site runs its alias and
`verify_agent_registry.py` reports **PIN-1** for the agents behind it.

Running an alias is a legitimate posture. Running one without having decided to is not —
so the warning surfaces in every build log, making the choice visible rather than default.
