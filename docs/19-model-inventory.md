...
| M8 | Conversation safety classifier + trajectory engine (Gemini Flash-Lite via the AI gateway, `ui/safety_signals.py`) | **B** | LLM (vendor foundation model) classifier + deterministic rule engine, control model | Classifies every customer turn (prompt and answer) against a closed taxonomy — security probes, scam/coercion, self-harm/distress, agent policy breach — folds it into the session, and decides hand-off / quarantine / human review in the request | Platform / AI governance | **High** — a control that changes what the customer is served (crisis and fraud hand-offs, locked sessions); its false negatives are the Raine failure, its false positives are a wrongly locked customer | Offline: ten scripted trajectories through the same engine, gated at 1.0 with zero late escalations (`safety_trajectories.jsonl`); classifier precision/recall against human-labeled turns not yet measured (gap) | Every screened turn recorded in `turn_signals` incl. classifier model and unscreened turns; tier-3 population baseline (`safety_buckets`, `scripts/safety_anomaly.py`) watches its own outage (`unscreened_rate`) and refusal collapse per `model_served` | [ADR-0034](adr/0034-conversation-safety-signals.md) |
