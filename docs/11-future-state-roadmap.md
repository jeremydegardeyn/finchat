...
| **Cold starts** | Scale-to-zero: first hit pays ~5–10s per service. | `min-instances ≥ 1` + **startup CPU boost** on Cloud Run (the `run_min_instances` Terraform variable already exists); the BFF's per-request HTTP clients become a shared connection pool (saves a TLS handshake per backend hop). |
