...
def test_a_bigquery_denial_is_named_as_data_access_and_sent_to_the_product_owner():
    out = server._analytics_denied(403, "PERMISSION_DENIED: user does not have "
...
def test_a_policy_tag_denial_is_named_as_data_access():
    out = server._analytics_denied(403, "Access Denied: policy tag pii_financial on "
...
def test_a_401_is_reported_as_a_session_problem_not_a_data_problem():
    ...
    out = server._analytics_denied(401, "UNAUTHENTICATED: invalid authentication credential")
...
def test_the_reason_is_always_kept():
    ...
    for status, body in ((403, "policy tag denied"), (403, "no entitlement"),
                         (401, "bad credential")):
        assert body in server._analytics_denied(status, body)["detail"]
