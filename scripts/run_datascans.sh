...
echo
echo "To surface the score on the catalog entry's operational aspect, re-run:"
echo "  python scripts/catalog_bootstrap.py ${ENV}"
echo "(its publish_dq_scores step reads the latest DQ job and writes the score,"
echo " pass/fail, and run time onto the silver-transaction entry's operational aspect.)"
